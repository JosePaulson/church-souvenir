...
const AddHouseScreen = () => {
  ...
  function handleImageChange(e) {
    ...
    displayImg.src = (e.target.files.length !== 0) ? window.URL.createObjectURL(e.target.files[0]) : 'https://user-images.githubusercontent.com/20684618/31289519-9ebdbe1a-aae6-11e7-8f82-bf794fdd9d1a.png' //sample image
  }
      ...
      <div className="flex items-center justify-center w-full gap-2 p-4 font-sans">
        <form className='grid grid-cols-1 gap-5 w-fit sm:grid-cols-2' onSubmit={handleAddHouse} >
          ...
          <Input
            ...
            required
          />
          <label className='row-span-3 w-fit ' htmlFor={!image ? 'image' : ''}><img id='displayImg' alt="" src='https://user-images.githubusercontent.com/20684618/31289519-9ebdbe1a-aae6-11e7-8f82-bf794fdd9d1a.png' className='h-24 rounded-lg sm:h-48 sm:w-72' /></label>
          <div className='relative'>
            <label htmlFor="image" className='absolute px-2 italic font-thin -top-1/4 right-2 bg-gray-50'>Add Image*</label>
            <FileInput id='image' className='w-full border-gray-400' type='file' onChange={handleImageChange} accept='image/*' required/>
          </div>
          <div className='relative'>
